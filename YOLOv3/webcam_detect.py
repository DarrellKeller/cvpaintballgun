...
ser = serial.Serial('/dev/ttyACM0', baudrate = 9600) #new

class YOLO(object):
    _defaults = {
        #"model_path": 'logs/trained_weights_final.h5',
        "model_path": 'model_data/yolot_weights.h5',
...
#darrell code
        def xcord(person):
            top, left, bottom, right, mid_v, mid_h, label, scores = person
            xmid = int((((mid_v)*(-160))/600)+180)
            return (xmid)
        def ycord(person):
            top, left, bottom, right, mid_v, mid_h, label, scores = person
            ymid = int(((mid_h)*180)/600)
            return (ymid)
        xcoord = [xcord(o) for o in ObjectsList if o[6].startswith('person')]
        ycoord = [ycord(o) for o in ObjectsList if o[6].startswith('person')]
        
        try:
            if xcoord != None :
                arduino = "X{}Y{}".format (xcoord[0],ycoord[0])
                print(arduino)
